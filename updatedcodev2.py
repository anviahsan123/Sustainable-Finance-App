...
This script implements the first phase of a Sustainable Finance App for retail investors.

The application asks the user for their risk preferences (gamma), ESG preferences (lambda),
and the characteristics of two risky assets (expected return, standard deviation, correlation,
risk‑free rate, and ESG rating). Using these inputs, it constructs the ESG‑efficient frontier
for the two risky assets, identifies the ESG‑adjusted tangency portfolio, and then determines
the optimal complete portfolio via a quadratic utility function that incorporates an ESG
preference term. Finally, it displays the recommended portfolio weights along with the
portfolio's expected return, risk, Sharpe ratio, and ESG score. Optionally, it can plot the
efficient frontier and mark both the tangency and optimal portfolios as well as the capital
market line.

Instructions:
1. Run this script in a Python environment with `numpy` and `matplotlib` installed.
2. Provide the requested inputs when prompted. Returns and standard deviations should be
   entered as percentages (e.g., 8 for 8%), and ESG ratings should be on a 0–100 scale.
3. The script will compute the optimal portfolio based on the inputs and display the results.

The utility function used is:

    U = E[R_p] − (γ/2)·σ_p² + λ·s̄

where s̄ is the weighted average ESG score of the risky assets. A higher λ implies a stronger
preference for greener portfolios, while a higher γ implies greater risk aversion. To avoid
borrowed leverage in the optimal portfolio, the proportion of the tangency portfolio is
